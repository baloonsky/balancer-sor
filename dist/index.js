...
class StablePool {
    ...
    subtractSwapFeeAmount(amount, swapFee) {
        ...
        const feeAmount = amount
            .mul(swapFee)
            .add(constants.WeiPerEther.sub(1))
            .div(constants.WeiPerEther);
    ...
    _tokenInForExactTokenOut(poolPairData, amount) {
        try {
            ...
            const returnAmtWithFee = this.addSwapFeeAmount(
                bignumber.BigNumber.from(returnAmountConverted),
                poolPairData.swapFee
            );
    ...
    _exactWrappedTokenInForBPTOut(poolPairData, amount) {
        try {
            ...
            const amt = _calcBptOutPerWrappedIn(
                bignumber.parseFixed(amount.toString(), 18).toBigInt(),
                poolPairData.mainBalanceScaled.toBigInt(),
                poolPairData.wrappedBalanceScaled.toBigInt(),
                poolPairData.virtualBptSupply.toBigInt(),
                {
...
function _safeLargePow3ADown(l, root3Alpha, d) {
    ...
    if (l.lte(_SAFE_LARGE_POW3_THRESHOLD)) {
        ...
        ret = ret
            .mul(l.div(constants.WeiPerEther))
            .add(
                ret.mul(l.mod(constants.WeiPerEther)).div(constants.WeiPerEther)
            );
        let x = ret;
        for (let i = 0; i < 3; i++) {
            x = x
...
function _calculateCubicStartingPoint(a, mb, mc) {
    ...
    const factor = alpha.gte(constants.WeiPerEther.div(2))
        ? constants.WeiPerEther.mul(3).div(2)
        : constants.WeiPerEther.mul(2);
...
function _calcOutGivenIn(balanceIn, balanceOut, amountIn, virtualOffset) {
    ...
    // computation.
    const virtInOver = balanceIn.add(
        mulUp(virtualOffset, constants.WeiPerEther.add(2))
    );
    const virtOutUnder = balanceOut.add(
        mulDown(virtualOffset, constants.WeiPerEther.sub(1))
    );
